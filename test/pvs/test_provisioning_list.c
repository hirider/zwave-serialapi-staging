...
static void test_provisioning_list_pseudo_tlv_set(void)
{
    ...
    provisioning_list_status_set(pvs, PVS_STATUS_DISABLED);
    check_true(pvs->status == PVS_STATUS_DISABLED, "status changed to DISABLED");

    start_case("Set status on non-existing device", log_strm);
    provisioning_list_status_set(NULL, PVS_STATUS_DISABLED);
    check_true(pvs->status == PVS_STATUS_DISABLED, "status changed to DISABLED");
    close_case("Set status on non-existing device");
