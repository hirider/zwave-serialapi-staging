...
)CC_REENTRANT_ARG
{
  ...
  //TODO: Find a way to pass the arguments more effeciently 

  SAPI_NodeInfoBridge(bNodeID, pCmd, bLen);
...
ApplicationIpCommandHandler(zwave_connection_t *c, void *pData, u16_t bDatalen) REENTRANT
{
  ...
  if(pCmd->ZW_NodeInfoCachedGetFrame.cmdClass == COMMAND_CLASS_NETWORK_MANAGEMENT_PROXY &&
    ...
    /* Always allow */
  } else if (pCmd->ZW_SupervisionGetFrame.cmdClass == COMMAND_CLASS_SUPERVISION &&
