...
#include "../SerialApiCustom/SerialapiEventHandler.h"
    ...
    ZW_APPLICATION_TX_BUFFER *pCmd, BYTE cmdLength)CC_REENTRANT_ARG
{
  ...
  SAPI_ResponseHandler(&p,pCmd, cmdLength);
...
#ifndef AURA_DEV
  process_exit(&tcpip_process); enable to restore TCPIP
...
#ifndef AURA_DEV
  /*Init UDP listners */
  process_exit(&udp_server_process); Enable to restore UDP server process
...
ZIP_Router_Reset() CC_REENTRANT_ARG
{
  ...
  if(!SerialAPI_GetRandom(sizeof(rnd), rnd)) {
...
//  ClassicZIPNode_init();
  ...
  process_exit(&dhcp_client_process); enable to restore dhcp_client
    ...
    while(1)
    {
      ...
      if(ev == serial_line_event_message)
      {
        ...
        switch (kbd_char)
        {
          ...
          case 'S':
          {
            /*this in-case we have to test endpoints manually
              have to enable SAPI_ApplicationHandler for this to work*/
            LOG_PRINTF("***************** Serial API Client *****************\n");
            SerialAPI_Poll();
            unsigned long long SAPIHexArray;
            printf("\nServices :  [01 Add Node]   [02 Remove Node]   [03 Send Single Channel]\
            \n   [04 Send Multichannel]   [05 Send Multi Device]   [06 Get node info]\
            \n   [07 Controller capability]   [08 Set Learn Mode]   [09 Get Network info]\
            \n   [0A Encrypt]   [0B Broadcast Node Info]   [0D Unit Test] \n");

            printf("\nData string info : [service id][Primary CC][Secondary CC][node id][endpoint id][State]\n");
            printf("\nEnter Data String : ");
            scanf(" %llx",&SAPIHexArray);
            /* TODO: tiny bug which is skipping the scanf statement when random words are entered 
            in the previous input */ 
            //SAPI_ApplicationHandler(&SAPIHexArray);
            process_post(&SAPI_Process,0xff,0);
          }
          break;
#endif //AURA_DEV
          default:
      ...
      else if (ev == PROCESS_EVENT_INIT) //imp
      {
        if (!ZIP_Router_Reset())
        {
      ...
      {
        LOG_PRINTF("************************** Exiting Serial Gateway **************************\n");
        LOG_PRINTF("Bye bye\n");
      }

      else if(ev == PROCESS_SERIAL_API)
      {
        process_exit(&SAPI_Process);
        process_start(&SAPI_Process,0);
      }
