...
bool ZW_NVM_Backup(const char* filename,uint8_t chiptype)
{
  ...
  */
  ...
  nvm_close_status = SerialAPI_nvm_close();
 ...
 /* On 700-series chips, SerialAPI_nvm_close must be followed by a chip reset */
...
bool ZW_NVM_Restore(const char* filename, uint8_t chiptype)
{
  ...
  */
  ...
  ZW_SetDefault(Set_Default_Callback);
  ...
  expected_length = SerialAPI_nvm_open();
  ...
  {
    write_status = SerialAPI_nvm_restore(offset, buffer, length_write, &length_written);
