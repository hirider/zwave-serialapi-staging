...
typedef struct SerialAPICpabilities {
  ...
  BYTE supported_serialapi_bitmask;
  BYTE nodelist[29];
  BYTE chipType;
  BYTE chipVersion;
...
{ 
  //This is actually correct as the commands are bitmasks this means that we
  // can only  support 8 commands
  return ( subcmd & capabilities.supported_serialapi_bitmask) > 0;
}
...
BOOL SerialAPI_Init(const char* serial_port, const struct SerialAPI_Callbacks* _callbacks )
{
  ...
  int i;
    ...
    if(SupportsCommand(FUNC_ID_SERIALAPI_SETUP)) {
      ...
      capabilities.supported_serialapi_bitmask = buffer[IDX_DATA + 1];
    }
    ...
    case FUNC_ID_ZW_APPLICATION_CONTROLLER_UPDATE:
      {
              ...
              (bStatus == UPDATE_STATE_NODE_INFO_SMARTSTART_HOMEID_RECEIVED_LR))
          {
            ...
            length = min(pData[idx + (j++)], sizeof pCmd);  //j is 2 (3 in LR)
    ...
    case FUNC_ID_APPLICATION_COMMAND_HANDLER:
      {
        if(callbacks->ApplicationCommandHandler) {
          ...
          uint8_t len = pData[ IDX_DATA + (j++) ]; //j is 2 (3 in LR)

          for ( i = 0;i < len;i++ )
              pCmd[ i ] = pData[ IDX_DATA + (j) + i ];//j is 3 (4 in LR)

          callbacks->ApplicationCommandHandler( pData[ IDX_DATA ], 0, source_node, (ZW_APPLICATION_TX_BUFFER*)pCmd, len);
        }
      }
      break;
    ...
    case FUNC_ID_APPLICATION_COMMAND_HANDLER_BRIDGE:
      {
        if(callbacks->ApplicationCommandHandler_Bridge) {
          ...
          len =  pData[IDX_DATA + (j++)]; //j is 3 ( 5 in LR)
          ...
          callbacks->ApplicationCommandHandler_Bridge(
            ...
            dest_node,
            source_node,
            (ZW_APPLICATION_TX_BUFFER*)pCmd, len);
        }
      }
      break;
   ...
   /*
    ...
    case FUNC_ID_ZW_SET_LEARN_MODE:
      if ( cbFuncZWSetLearnMode != NULL )
      {
          ...
          if (DetectBufferOverflow(IDX_DATA + (j))) //j is 3 (4 in LR)
            break;
    ...
    case FUNC_ID_SERIALAPI_STARTED:
      ...
      if (my_chip_data.chip_type == ZW_GECKO_CHIP_TYPE) {
 ...
 */
uint8_t SerialAPI_nvm_backup(uint32_t offset, uint8_t *buf, uint8_t length, uint8_t *length_read)
{
  int i;
...
uint8_t SerialAPI_nvm_restore(uint32_t offset, uint8_t *buf, uint8_t length, uint8_t *length_written)
{
  int i;
    ...
    if(retVal == conFrameReceived) {
        if (pBuf)
        {
          ...
          switch (byLen - IDX_DATA) {
            ...
            case 6:
              pBuf->zaf_build_no = buffer[IDX_DATA+4]; 
...
//#include "../transport/rijndael-alg-fst.h"
