...
activate_image(uint16_t firmware_id, uint8_t firmware_target, uint16_t crc, uint32_t firmware_len, uint8_t activation)
{
  ...
  status = FALSE;
  if (activation && ((chip_desc.my_chip_type != ZW_GECKO_CHIP_TYPE) && (firmware_id != ZW_FW_ID))) {
    ...
    if (desc.firmware_id != firmware_id || desc.target != firmware_target || desc.crc != crc) { // We dont need this check if its ZW_FW_ID for ZW_GECKO_CHIP_TYPE (700 series)
