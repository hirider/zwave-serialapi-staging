  ...
  if(cur_send_ima && (flags0 & (ZIP_PACKET_FLAGS0_ACK_RES | ZIP_PACKET_FLAGS0_NACK_RES) ))
  {
    ...
    *p++ = 5;
    ...
    if (NULL != txStat) {
      ...
      *p++ = RSSI_NOT_AVAILABLE;
      *p++ = RSSI_NOT_AVAILABLE;
      assert((p - ima_buffer) == sizeof(ima_buffer));
    } else {
...
int ClassicZIPNode_dec_input(void * pktdata, int len)
{
  ...
  DBG_PRINTF("ClassicZIPNode_dec_input: pkt: %x %x len: %d\n", cc[0], cc[1], len);
...
CreateLogicalUDP(ts_param_t* p, unsigned char *pCmd, uint8_t cmdLength)
{
  ...
  if (a)
  {
    ...
    DBG_PRINTF("Packet from Z-wave side (nodeid: %d) to port: %d IP addr: ", p->snode, UIP_HTONS(c.rport));
