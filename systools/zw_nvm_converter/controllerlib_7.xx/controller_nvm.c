...
static uint8_t nvm3_storage[TOTAL_NVM_SIZE + FLASH_PAGE_SIZE];
...
bool check_controller_nvm(const uint8_t *nvm_image, size_t nvm_image_size)
{
  ...
  if(!open_controller_nvm(nvm_image,nvm_image_size)) return false;
...
void dump_controller_nvm_keys(void)
{
  if (nvm3_app_handle.hasBeenOpened)
  {
    ...
    if ((target_protocol_version.minor < 15) ||
      ((target_protocol_version.minor == 15) && (target_protocol_version.patch < 3))) {
...
static json_object* app_config_to_json(void)
{
  ...
  json_object* jo = json_object_new_object();
  if ((target_protocol_version.major == 7)
    && (target_protocol_version.minor == 15)
    && (target_protocol_version.patch >= 3)) {
  ...
  if(target_protocol_version.format >= 2) {
    for (uint16_t node_id = 1; node_id <= ZW_LR_MAX_NODES; node_id++)
    {
      if (ZW_NodeMaskNodeIn(lr_node_info_exists, node_id))
      {
        ...
        /* LR Tx Power */
        ec = READ_PROT_NVM(FILE_ID_LR_TX_POWER + node_index / LR_TX_POWER_PER_FILE, lr_tx_power_buf);
        lr_tx_power = &lr_tx_power_buf[(node_index % LR_TX_POWER_PER_FILE)/2];
        if (ECODE_NVM3_OK == ec)
        {
          if ((node_id -1) & 1)
          {
  ...
  for (int i = 0; (i < json_object_array_length(jo_ntbl)) && (i < ZW_LR_MAX_NODE_ID); i++)
  {
    ...
    if (node_id > 0 && node_id <= ZW_CLASSIC_MAX_NODES)
    {
      ...
      if (target_protocol_version.format >= 2) {
        ...
        /* LR Tx Power */
        file_id = FILE_ID_LR_TX_POWER + (node_id - ZW_LR_MIN_NODE_ID)/LR_TX_POWER_PER_FILE;
        ec = nvm3_getObjectInfo(&nvm3_protocol_handle, file_id, &file_type, &file_len);
        if (ec == ECODE_NVM3_OK) {
          READ_PROT_NVM(file_id, lr_tx_power_buf);
        }
        lr_tx_power = &lr_tx_power_buf[((node_id - ZW_LR_MIN_NODE_ID) % LR_TX_POWER_PER_FILE) / 2];
                                                            ...
                                                            , uint8_t patch_app_version)
{
   ...
   */
  if (7 == major_app_version &&
     15 == minor_app_version &&
      3 <= patch_app_version)
  {
    SApplicationConfiguration ac = {};
