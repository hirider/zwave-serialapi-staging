...
/* 128 byte bitmask in NVM can store at most 1024 LR nodes, which counts from 256
 * 128 * 8 + 256 - 1 = 1279
 */
#define ZW_LR_MAX_NODE_ID_IN_NVM   1279
...
#define NODE_MULTICAST              0xFE    /* multicast */
...
typedef struct _TX_STATUS_TYPE_
{
  ...
  BYTE bUsedTxpower;
  BYTE bMeasuredNoiseFloor;
  BYTE bAckDestinationUsedTxPower;
  BYTE bDestinationAckMeasuredRSSI;
  BYTE bDestinationckMeasuredNoiseFloor;
} TX_STATUS_TYPE;
...
#define ZW_SendData(node,data,length,options,func) ZW_SendData_Bridge(0xffff,node,data,length,options,func)
...
ZW_SendData_Bridge(
  uint16_t  bSrcNodeID,           /*IN  Source NodeID - if 0xFF then controller ID is set as source */
...
** This function must be called for each active channel. In 2CH systems active
** channels are  0 and 1. In 3-channel systems (JP and KR), 
** call for ch. 0, 1 and 2.
**
** \param bChannel Channel number. The Threshold should be set for. Valid
**                   channel numbers are 0,1 and 2
** \param bThreshold The threshold the RSSI should use. Valid threshold range
**                   is from 34(dec) to 78(dec).
**    Side effects:
**
**--------------------------------------------------------------------------*/
